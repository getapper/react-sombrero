...
module.exports = async () => {
  ...
  if(isRoute) {
    ...
    if(routerIndex === 0) {
      insertInRouter(dirname, sceneName, initialAccess, hash, key, isRouter, dirname, routerName, path.join(indexNamePair[directoryNumber].path, `scenes/${sceneName}`), routerIndex)
    } else {
      insertInRouter(indexNamePair[routerIndex].path, sceneName, initialAccess, hash, key, isRouter, dirname, routerName, path.join(indexNamePair[directoryNumber].path, `scenes/${sceneName}`), routerIndex)
    }
  }
