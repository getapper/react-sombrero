...
module.exports = async argv => {
  ...
  components.init(capitalize(_.camelCase(componentName)), hasRedux, isFunctional)
  ...
  if(directoryNumber === 0) {
    ...
    directories.create(path.join(dirname, `src/components/${componentName}`))
    components.save(path.join(dirname, `src/components/${componentName}`))
    cssFiles.save(path.join(dirname, `src/components/${componentName}/style.scss`))
    cssFiles.init('css/base-style-js-template.ejs')
    cssFiles.save(path.join(dirname, `src/components/${componentName}/style.js`))
  } else {
    ...
    cssFiles.init('css/base-style-js-template.ejs')
    cssFiles.save(path.join(indexNamePair[directoryNumber].path, `components/${componentName}/style.js`))

    // Insert component in parent directory
