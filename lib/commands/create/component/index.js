...
module.exports = async argv => {
  ...
  if (directoryNumber === 0) {
    ...
    basePath = `src/components/${componentName}`
  } else {
  ...
  directories.create(path.join(dirname, basePath))
  components.save(path.join(dirname, basePath))
  if (useScss) {
    cssFiles.init('css/base-css-template.ejs')
    cssFiles.save(path.join(dirname, `${basePath}/style.scss`))
  }
  if (useJss) {
    cssFiles.init('css/base-style-js-template.ejs')
    cssFiles.save(path.join(dirname, `${basePath}/style.js`))
  }
