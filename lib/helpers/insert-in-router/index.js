...
function insertInRouter(routerPath, sceneName, initialAccess, hash, key, isRouter, dirname, routerName, scenePath, routerIndex) {
  ...
  let insertString = `\nimport ${capitalize(_.camelCase(sceneName))} from '${relativePath}'`
  ...
  if(isRouter) {
    if (!fs.existsSync(path.join(dirname, 'redux/initial-states/routing'))) {
      directories.create(path.join(dirname, 'redux/initial-states/routing'))
    }
    if(!fs.existsSync(path.join(dirname, 'redux/initial-states/routing/index.js'))) {
      commonJsFile.init('redux/init/initial-states-routing-template.ejs')
      commonJsFile.save('redux/initial-states/routing/index.js')
    }
    if (!fs.existsSync(path.join(dirname, 'redux/actions/routing'))) {
      directories.create(path.join(dirname, 'redux/actions/routing'))
    }
    if(!fs.existsSync(path.join(dirname, 'redux/actions/routing/index.js'))){
