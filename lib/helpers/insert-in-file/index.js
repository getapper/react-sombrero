...
const insertInRouter = (routerPath, sceneName, initialAccess, hash, key, isRouter, dirname, routerName, scenePath, routerIndex) => {
  ...
  const bracketsIndex = file.indexOf('>{}')
  if(bracketsIndex !== -1) {
    file = file.slice(0, insertIndex - 2).concat(file.slice(insertIndex))
    file = file.slice(0, insertIndex).concat(route).concat('\n').concat(file.slice(insertIndex))
  } else {
...
const insertInRedux = (dirname, routerName) => {
...
const insertInParent = (parentPath, componentName) => {
  let file = scenes.read(path.join(parentPath, 'index.js'))
  let insertString = `\nimport ${capitalize(_.camelCase(componentName))} from './components/${componentName}'`
  let insertIndex = file.indexOf('\n\n', file.indexOf('// REACT'))
  if (insertIndex === -1) {
    insertIndex = file.indexOf('\r\n\r\n', file.indexOf('// REACT'))
  }
  file = file.slice(0, insertIndex).concat(insertString).concat(file.slice(insertIndex))
  create(path.join(parentPath, 'index.js'), file)
}

const insertInInitialState = (dirname, sliceName) => {
  let insertString = `\nimport ${sliceName} from './${sliceName}'`
  let file = jsFiles.read(path.join(dirname, `src/redux/initial-states/index.js`))
  let insertIndex = file.indexOf('\n\n', file.indexOf('// REDUX'))
  if (insertIndex === -1) {
    insertIndex = file.indexOf('\r\n\r\n', file.indexOf('// REDUX'))
  }
  ...
  insertString = `  ${sliceName},\n`
  insertIndex = file.indexOf('})', file.indexOf('export default immutable.Map({'))
  file = file.slice(0, insertIndex).concat(insertString).concat(file.slice(insertIndex))

  create(path.join(dirname, 'src/redux/initial-states/index.js'), file)
}

const insertInReducers = (dirname, sliceName) => {
  let insertString = `\nimport ${sliceName} from './${sliceName}'`
  let file = jsFiles.read(path.join(dirname, `src/redux/reducers/index.js`))
  let insertIndex = file.indexOf('\n\n', file.indexOf(`import routing from './routing'`))
  if (insertIndex === -1) {
    insertIndex = file.indexOf('\r\n\r\n', file.indexOf(`import routing from './routing'`))
  }
  file = file.slice(0, insertIndex).concat(insertString).concat(file.slice(insertIndex))
  ...
  insertString = `  ${sliceName},\n`
  insertIndex = file.indexOf('}', file.indexOf('export default {'))
  file = file.slice(0, insertIndex).concat(insertString).concat(file.slice(insertIndex))

  create(path.join(dirname, 'src/redux/reducers/index.js'), file)
}
